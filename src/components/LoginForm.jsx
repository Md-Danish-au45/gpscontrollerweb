...
//   return (
//     <div className="min-h-screen  via-white to-cyan-50 flex items-center justify-center p-4 relative overflow-hidden">


//       <div className="w-260 max-w-md relative z-10">
...
// New FloatingLabel component with improved state synchronization
const FloatingLabel = ({ label, value, onChange, onBlur, error, icon: Icon, type = "text", showPassword, onTogglePassword }) => {
  ...
  return (
    <div className="relative">
      <input
        ...
        onFocus={handleFocus}
        onBlur={handleBlur}
        className={`peer w-full h-12 pt-4 px-4 text-gray-900 bg-gray-50 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 ${error ? "border-red-500 focus:ring-red-500" : ""}`}
        placeholder=" "
        autoComplete={type === "password" ? "current-password" : "off"}
      />
      <label
        className={`absolute left-4 transition-all duration-200 ease-in-out pointer-events-none text-gray-500 ${isFocused || isFilled ? "top-2 text-xs" : "top-1/2 -translate-y-1/2 text-base"}`}
      >
        {label}
      </label>
      <div className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 peer-focus:text-blue-500 transition-colors duration-200">
...
export function LoginForm() {
  ...
  // New: Handle password autofill onBlur
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ apiError: err?.data?.message || "Failed to log in. Please check your credentials and try again." });
  ...
  const handleForgotPasswordSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ apiError: err?.data?.message || "An error occurred. Please try again." });
      console.error("Forgot password request failed:", err);
    }
  };
